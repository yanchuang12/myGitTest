...
int CXTabCtrl::AddTab(CWnd* pWnd, LPTSTR lpszCaption, int iImage)
{
	ASSERT_VALID(pWnd);
	...
	pWnd->SetWindowPos(&wndTop, m_ptTabs.x, m_ptTabs.y , 0, 0,
...
void CXTabCtrl::AdjustSize()
{
	int nCnt = m_arrayStatusTab.GetSize();
	if(nCnt < 1)	
		return;	
	CRect Rect;
	GetClientRect(&Rect);
	for(int i=0; i<nCnt; ++i)
	{
		...
		if(0 == item.lParam)
			return;
		((CWnd*)item.lParam)->SetWindowPos(&wndTop, m_ptTabs.x, m_ptTabs.y , 
...
void CXTabCtrl::OnSizing(UINT fwSide, LPRECT pRect) 
{
	CTabCtrl::OnSizing(fwSide, pRect);
