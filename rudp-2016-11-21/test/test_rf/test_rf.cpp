...
public:
	...
	bool folder_recv(char cflag, uint64_t &recv_total_size, unsigned long &tm)
	{
		...
		while (1)
		{
			if (HDR_LEN != ndrudp_recv(this->s_, buf, HDR_LEN, -1)) { 
			...
			if (nbody != ndrudp_recv(this->s_, buf, nbody, -1)) {
			...
			else
			{
				...
				while (1) 
				{
					...
					if (HDR_LEN != ndrudp_recv(this->s_, buf, HDR_LEN, -1) || buf[2] != 2) {
						printf("ndrudp_recv head error\r\n");
						return false;
					}
					...
					int rcv_len = ndrudp_recv(this->s_, buf, nbody, -1);
					if (rcv_len != nbody) {
	...
	bool __svc()
	{
		...
		int tmo = 10*1000;
		char cflag = '\0';
		if (HDR_LEN != ndrudp_recv(this->s_, buf, HDR_LEN, -1) || buf[2] != 1) { 
			printf("ndrudp_recv head error\r\n");
printf("buf[2] =%c...\n",buf[2]);
		...
		else
		{
			...
			while (1) {
				...
				if (HDR_LEN != ndrudp_recv(this->s_, buf, HDR_LEN, -1) || buf[2] != 2) {
					printf("ndrudp_recv body head error\r\n");
					return false;
				}
		...
		*((uint16_t*)buf) = htons(0);
		...
		if (HDR_LEN != ndrudp_send(this->s_, buf, HDR_LEN, -1)) {
