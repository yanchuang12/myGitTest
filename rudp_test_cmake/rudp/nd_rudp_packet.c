...
nd_rudp_packet_parse(nd_rudp_packet_t *pkt)
{
	...
	pkt->hdr.unused = data[0];
	pkt->hdr.exhdr_len = data[1];
	...
	pkt->hdr.flag = ntohs(((nd_uint16_t*)data)[7]);

	pkt->hdr.vir_seq = ntohs(((nd_uint16_t*)data)[8]);
	pkt->hdr.unused2 = ntohs(((nd_uint16_t*)data)[9]);
	...
	if (pkt->hdr.exhdr_len == 0) {
		...
		pkt->hdr.exhdr = data + ND_RUDP_HDR_LEN;
	}
...
nd_rudp_packet_write(nd_rudp_packet_t *pkt, int only_hdr)
{
	...
	data[0] = pkt->hdr.unused;
	data[1] = pkt->hdr.exhdr_len;
	...
	((nd_uint16_t*)data)[7] = htons(pkt->hdr.flag);

	((nd_uint16_t*)data)[8] = htons(pkt->hdr.vir_seq);
	((nd_uint16_t*)data)[9] = htons(pkt->hdr.unused2);
