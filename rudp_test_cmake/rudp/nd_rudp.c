...
nd_rudp_endpoint_t*
nd_rudp_sock2ep(void* s)
{
	return (nd_rudp_endpoint_t*)s;
}
...
static int ND_CALL_PRO __event_thread(void *p)
{
	...
	while (!rudp->th.shut_down) {
		...
		if (rudp->ch && rudp->ch->handle_refresh_cb) {
			rudp->ch->handle_refresh_cb(rudp->ch, rudp->ch->s);
		}
	}
#ifdef __ND_WIN32__
	timeEndPeriod(1);
...
__nd_rudp_process_packet(nd_rudp_event_t *ev, nd_rudp_packet_t *pkt)
{
	...
	if (ev->ch->svr) {
		if (pkt->hdr.token) {
			...
			ep = nd_rudp_channel_find_ep2(ev->ch, &pkt->so_addr);
			if (!ep && (pkt->hdr.flag & ND_RUDP_HDR_FLAG_SHAKEHAND) && !(pkt->hdr.flag & ND_RUDP_HDR_FLAG_ACK)) {
				...
				ep->ch = ev->ch;
				nd_rudp_ip_copy(&ep->so_remote_addr, &pkt->so_addr, ev->ch->ip_version);
				/// 分配token
	...
	/// ip地址变了? 也存在这种可能
	if (!nd_rudp_ip_equal(&ep->so_remote_addr, &pkt->so_addr, ep->ch->ip_version)) {
		/// 重新设置对端地址
		nd_rudp_ip_copy(&ep->so_remote_addr, &pkt->so_addr, ep->ch->ip_version);
	}
...
__nd_rudp_handle_read(nd_rudp_event_t *ev)
{
	...
	nd_int_t i;
	nd_uint64_t curr_tm = nd_get_ts();
	for (i = 0; i < __ONE_CYCLE_MAX_RCV_PKT_CNT__ && (nd_get_ts() - curr_tm < 5000); i++) {

		if (ev->ch->closed) {
			break;
		}

		pkt = nd_rudp_packet_create(1500);
		if (!pkt) {
			break;
		}

		r = nd_rudp_channel_recv_from(ev->ch, pkt, &pkt->so_addr);
		if (r == ND_RET_ERROR) {
			nd_rudp_packet_destroy(pkt);
...
nd_rudp_process_ctrl(nd_rudp_endpoint_t *ep, nd_rudp_packet_t *pkt)
{
	...
	///更新速率
	if (pkt->hdr.flag & ND_RUDP_HDR_FLAG_BW_RATE) {
		if (pkt->hdr.exhdr_len >= 8) {
			nd_uint32_t rate, bw;
	...
	if ((pkt->hdr.flag & ND_RUDP_HDR_FLAG_REPEAT_WARN) && ep->cc_mode != ND_RUDP_CC_MODE_FIXED_RATE) {
		/// 减速
		ep->trace_info.repeat_warn ++;
		ep->trace_info.total_repeat_warn ++;

		if (ep->loss_rate < ep->loss_threshold) {
			if (ep->cc.inter_usec > ep->cc.last_dec_inter_usec) {
				ep->cc.monitor.s_repeat_keep += 5;
			}
			else {
				ep->cc.monitor.s_repeat_keep += 10;
			}
		}
		else {
			if (ep->cc.inter_usec > ep->cc.last_dec_inter_usec) {
				ep->cc.monitor.s_repeat_keep += 50;
			}
			else {
				ep->cc.monitor.s_repeat_keep += 100;
			}
		}
...
__nd_rudp_handle_write(nd_rudp_event_t *ev)
{
	...
	if (ev->ch->svr) {  
		/// add 针对多个连接
		nd_int_t sum_pkt_inshort = 0;
		nd_int_t m = __ONE_CYCLE_MAX_SND_PKT_CNT__ / nd_max(1, ev->ch->eps.size);
		nd_int_t average_pkt_snd;
		nd_int_t i = 0;
		nd_dlist_node_t *curr;
		if (ev->ch->eps.size > 1) {
			if (m < 1)
				m = 1;
			...
			nd_queue_for_each(nd, &ev->ch->eps.header) {
				...
				if (ep->s_que.size < m) {
					sum_pkt_inshort += ep->s_que.size;
					i++;
				}
			}
			
			if (ev->ch->eps.size == i)  // 都比m小
				average_pkt_snd = m;
			else
				average_pkt_snd = (__ONE_CYCLE_MAX_SND_PKT_CNT__ - sum_pkt_inshort) / (ev->ch->eps.size - i);

			if (average_pkt_snd < 1)
				average_pkt_snd = 1;

			nd_queue_for_each(nd, &ev->ch->eps.header) {
				nd_rudp_endpoint_t *ep = nd->data;
				nd_rudp_endpoint_do_send(ep, NULL, 0, average_pkt_snd);
				if (ev->ch->wr_pending) {
					break;
				}
			}
			curr = (&ev->ch->eps.header)->next;
			nd_queue_remove(curr);
			nd_queue_insert_tail(&ev->ch->eps.header, curr);
		}
		else {
			nd_queue_for_each(nd, &ev->ch->eps.header) {
				nd_rudp_endpoint_t *ep = nd->data;
		...
		nd_rudp_endpoint_do_send(ev->ch->self, NULL, 0, -1);
	}
}
...
__nd_rudp_handle_locked_posted_event(nd_rudp_locked_posted_event_t *lev)
{
	...
	case ND_RUDP_LOCKED_MSG_CLOSE_EP: {
		...
		break;
	case ND_RUDP_LOCKED_MSG_SHUTDOWN_EP: {
			nd_rudp_endpoint_t *ep = (nd_rudp_endpoint_t*)lev->param;
			nd_rudp_endpoint_on_dead(ep);
		}
		break;
...
nd_rudp_process_data(nd_rudp_endpoint_t *ep, nd_rudp_packet_t *pkt)
{
	...
	nd_int_t pos, loss, size, lst_size;
	nd_uint8_t pkt_flag;
	nd_uint64_t curr, pkt_ts, pkt_rcv_tm;
	nd_uint16_t reply_fg;
	nd_int16_t vir_diff;
	...
	pkt_ts = pkt->hdr.ts;
	pkt_rcv_tm = pkt->rcv_tm;

	/// 确保分组序号在有效区间
	if (0 == nd_rudp_chk_dateseq(ep, pkt->hdr.seq)) {
		ep->trace_info.out_of_range ++;
		ep->trace_info.total_out_of_range ++;
		ep->cc.monitor.r_repeat_cnt++;
	...
	if (size >= ep->max_syn_seq_range) {
		...
		nd_rudp_log(ND_RUDP_LOG_LV_DEBUG, "data packet<sn=%u> out of wnd, size=%d", pkt->hdr.seq, size);
		nd_rudp_send_ctrl(ep, pkt->hdr.ts, ND_RUDP_HDR_FLAG_STAGED_ACK, pkt->rcv_tm);
		nd_rudp_packet_destroy(pkt);
		return;
	}

	vir_diff = pkt->hdr.vir_seq - ep->r_last_max_vir_seq;
	if (vir_diff > 0 && vir_diff < 0x7fff) {
		ep->r_last_max_vir_seq = pkt->hdr.vir_seq;
	}
	...
	if (find) {
		...
		nd_rudp_send_ctrl(ep, pkt_ts, ND_RUDP_HDR_FLAG_STAGED_ACK, pkt_rcv_tm);
	...
	/// 更新速率预估相关参数
	curr = nd_get_ts();
	nd_rudp_endpoint_on_rcv_packet(ep, pkt, curr);
	...
	/// 注意: @pkt在此可能已被释放, 不能再使用
	*p = NULL;
	

	/// PUSH标志将触发立即返回ack, 注意: 这里要先等待@ep->r_next_seq 更新后再ACK
	if (ep->r_staged_pkt_cnt >= ep->max_staged_pkt_cnt ||
		lst_size > 15 || /// 分组乱序时, 一个延迟到达的最小未确认序号分组, 可能会使得大量分组被累积确认, 滑动窗口右滑, 此时, 应该立即返回ack
		(nd_seq_diff(ep->r_last_max_seq, ep->r_next_seq) >= (ep->max_syn_seq_range - 16) && (nd_seq_diff(ep->r_last_max_seq, ep->r_next_seq) - (ep->max_syn_seq_range - 16)) % 4 == 0) ||
		(/*lst &&*/ (pkt_flag & ND_RUDP_HDR_FLAG_DUP)) ||		/// 如果是一个重传的分组,并有连续数据被确认
		(pkt_flag & ND_RUDP_HDR_FLAG_PUSH)) {

			reply_fg |= ND_RUDP_HDR_FLAG_STAGED_ACK;
	}

	if ((pkt_flag & ND_RUDP_HDR_FLAG_PROBE) && 
		!(pkt_flag & ND_RUDP_HDR_FLAG_ACK)) {
			reply_fg |= ND_RUDP_HDR_FLAG_PROBE | ND_RUDP_HDR_FLAG_ACK;
	}

	if (reply_fg) {
		nd_rudp_send_ctrl(ep, pkt_ts, reply_fg, pkt_rcv_tm);
	}
	...
	ep->r_ppp.ts_dev = ep->r_pkt_tm_wnd.ts_dev = nd_tm_u64_diff(nd_get_ts(), curr);
}

void 
nd_rudp_send_ctrl(nd_rudp_endpoint_t *ep, nd_uint32_t ts, nd_uint16_t flag, nd_uint64_t org_tm)
{
	nd_rudp_packet_t *pkt;
	nd_uint8_t *org_ptr;
	nd_int_t wnd;
	...
	pkt->hdr.vir_seq = ep->r_last_max_vir_seq;
	pkt->hdr.flag = flag;
	...
	if (flag & ND_RUDP_HDR_FLAG_REPEAT_WARN) {
		;
	}
	...
	if (pkt->hdr.flag & ND_RUDP_HDR_FLAG_PROBE) { 
		if (pkt->hdr.flag & ND_RUDP_HDR_FLAG_ACK) {
			...
			/// 更新探测时间
			ep->last_probe_tm = curr;
			ep->probe_ack_tmo = ep->last_probe_tm + nd_max(3 * ep->rto, ep->rto_min);
		}
	}

	if (nd_tm_u64_diff(curr, ep->last_ack_bw_rate_tm) > ep->cc.cycle_tm) {
		nd_uint32_t rate, bw;
		nd_uint8_t *ptr = org_ptr + ND_RUDP_HDR_LEN + pkt->hdr.exhdr_len;
		/// 携带速率
		...
		*(nd_uint32_t*)(ptr + 4) = htonl(bw);
		...
		ep->last_ack_bw_rate_tm = curr;

		pkt->hdr.flag |= ND_RUDP_HDR_FLAG_BW_RATE;
	}
	...
	if (flag & ND_RUDP_HDR_FLAG_STAGED_ACK) 
	{
		...
		*cnt_ptr = cnt1;
		...
		if (n > 0) {
			if (n > (nd_int32_t)ep->max_staged_ack_cnt2)
				n = (nd_int32_t)ep->max_staged_ack_cnt2;
...
do_build:
	...
	/*if ((flag & ND_RUDP_HDR_FLAG_BYE) ||
		(flag & ND_RUDP_HDR_FLAG_SHAKEHAND))*/ {
			...
			nd_rudp_channel_send_to(ep->ch, pkt, &pkt->so_addr);
			nd_rudp_packet_destroy(pkt);
			return;
	}
...
static void
__nd_rudp_check_sendlst(nd_rudp_endpoint_t *ep, nd_uint32_t seq, nd_int_t num, nd_uint8_t *bitmap, nd_uint32_t range_begin_seq, nd_uint32_t *chk_cnt, nd_int_t chk_loss, nd_uint64_t curr_tm)
{
			...
			for (j = 0; j < k; j++) {
				...
				if (p[i] & (0x01 << j)) {	/// bit == 1		
					if (pkt) {
						...
						/// 更新最大已确认序号
						if (nd_seq_cmp(pkt->hdr.seq, ep->s_max_ack_seq) > 0)
							ep->s_max_ack_seq = pkt->hdr.seq;
						...
						ep->trace_info.total_ack_bytes += pkt->hdr.body_len;
					...
					if (chk_loss && pkt && !pkt->in_loss_lst && pkt != ep->s_curr_pkt) {
						nd_rudp_endpoint_chk_loss(ep, pkt, 1, curr_tm);
					}
				}
			}
		}
	...
	/*if (chk_loss && seq_lst_cnt == SEQ_LST_SIZE) {
		...
		if (nd_seq_cmp(seq, min_seq) < 0) {
			for (s = seq; s != min_seq && cnt < num; s = nd_seq_inc(s), cnt++) {
				...
				if (pkt && !pkt->in_loss_lst && pkt != ep->s_curr_pkt) {
					nd_rudp_endpoint_chk_loss(ep, pkt, 1, curr_tm);
				}
			}
		}
...
nd_rudp_process_staged_ack(nd_rudp_endpoint_t *ep, nd_rudp_packet_t *pkt, nd_uint8_t *ptr, nd_int_t len)
{
	...
	*/
	...
	ep->trace_info.total_ack_pkt_cnt++;
		...
		if (nd_seq_between(pkt->hdr.seq, ep->s_next_ack_seq, nd_seq_inc(ep->s_next_seq))) {
			...
			for (s = ep->s_next_ack_seq; nd_seq_cmp(s, pkt->hdr.seq) < 0; s = nd_seq_inc(s)) {
				...
				if (p) {
					...
					ep->trace_info.total_ack_bytes += p->hdr.body_len;
			...
			/// 更新最大已确认序号
			if (nd_seq_cmp(pkt->hdr.seq, ep->s_max_ack_seq) > 0)
				ep->s_max_ack_seq = nd_seq_dec(pkt->hdr.seq);

			/// 更新最大虚拟序号
			vir_diff = pkt->hdr.vir_seq - ep->s_max_vir_seq;
			if (vir_diff > 0 && vir_diff < 0x7ffe) {
				ep->s_max_vir_seq = pkt->hdr.vir_seq;
			}
		}
		else {
			nd_rudp_log(ND_RUDP_LOG_LV_BUG, "nd_rudp_process_staged_ack bug");
		}
	...
	if (cnt1 > 0) {
		/// 仅staged ack1 检查loss
		__nd_rudp_check_sendlst(ep, pkt->hdr.seq, cnt1, ptr + 4, old_next_ack_seq, &chk_cnt, 1, curr_tm);
	}
	...
	if (cnt2 > ep->max_staged_ack_cnt2 || cnt2 < 0) {
		nd_assert(0);
		ret = ND_RET_ERROR;
		goto end;
	}
	...
	if (cnt2 > 0) {
		...
		__nd_rudp_check_sendlst(ep, staged_ack, cnt2, ptr + 12, old_next_ack_seq, &chk_cnt, 0, curr_tm);
	}
	...
	if (old_next_ack_seq != ep->s_next_ack_seq) {
		...
		if (p) {
			nd_rudp_endpoint_chk_loss(ep, p, 0, curr_tm);
		}

		/// 2. 新的数据被ack, 唤醒外部send调用
		nd_mutex_lock(&ep->s_que.mutex);
		if (ep->s_que.free_size == 0) {
			nd_int32_t n = nd_seq_diff(ep->s_next_ack_seq, old_next_ack_seq);
			if (ep->s_que.free_size + n > ep->s_que.max_free_size)
				ep->s_que.free_size = ep->s_que.max_free_size;
			else 
				ep->s_que.free_size += n;
		...
		nd_rudp_packet_t *p = ep->s_buf_lst[ep->s_next_ack_seq % ep->max_syn_seq_range];
		if (p) {
			nd_rudp_endpoint_chk_loss(ep, p, 2, curr_tm);
		}
	}
	if (ep->s_zero_wnd) {
