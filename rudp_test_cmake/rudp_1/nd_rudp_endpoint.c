...
nd_rudp_endpoint_init(nd_rudp_endpoint_t *ep)
{
	...
	ep->isyn_seq = 1;//(nd_uint32_t)nd_get_ts();
	if (ep->isyn_seq == 0)
		ep->isyn_seq = 1;
...
__handle_idle_timer_out(nd_tq_node_t *node)
{
	...
	if (ep->status == ND_RUDP_EP_STATUS_CONNECTED) {
		...
		if (ep->last_rcv_tm && nd_tm_u64_diff(curr, ep->last_rcv_tm) > 15*1000*1000) {
			nd_rudp_send_ctrl(ep, (nd_uint32_t)nd_get_ts(), ND_RUDP_HDR_FLAG_BYE, 0);
			nd_rudp_endpoint_close(ep);
			return;
		}
		...
		if (1) {
			...
			else if (ep->s_zero_wnd && nd_tm_u64_diff(curr, ep->last_probe_tm) > nd_min(ep->rtt + ep->cc.cycle_tm, 200000)) {
				/// 发送窗口为0, 每2个RTT,或者200msec探测一次,这个时间以及探测是否合理,必要???
				flag |= ND_RUDP_HDR_FLAG_PROBE;
			}
			else if (nd_tm_u64_cmp(curr, ep->last_probe_tm + ep->cc.cycle_tm) > 0) {
				/// 定时探测
				flag |= ND_RUDP_HDR_FLAG_PROBE;
			}
			...
			if (ep->probe_ack_tmo && nd_tm_u64_cmp(curr, ep->probe_ack_tmo) > 0) {
					...
					if (ep->cc.slow_start) {
						...
						ep->cc.cwnd = ep->cc.left_wnd + (ep->cc.cycle_tm + ep->rtt - (curr - ep->cc.begin_tm))/ep->cc.inter_usec;
...
void
nd_rudp_endpoint_chk_loss11(nd_rudp_endpoint_t *ep, nd_rudp_packet_t *pkt, nd_int_t flag)
{
	nd_uint64_t curr;
	nd_int_t loss;
	curr = nd_get_ts();
	...
	if (pkt) {
		...
		while (i++ != 32) {
			...
			if (pkt && pkt->loss_tmo && nd_tm_u64_cmp(curr, pkt->loss_tmo) > 0) {
				if (!pkt->loss) {
					if (++pkt->loss_cnt > 2) {
						nd_rudp_endpoint_add_loss_pkt(ep, pkt);
						loss = 1;
...
nd_rudp_on_loss(nd_rudp_endpoint_t *ep, nd_rudp_packet_t *pkt)
{
	...
	if (ep->cc.slow_start) {
		...
		if (ep->cc.rate > 0)
			...
			ep->cc.inter_usec = ((double)(ep->rtt + ep->cc.cycle_tm)) / ep->cc.cwnd;
	...
	if (ep->cc.inter_usec > 1000){
		ep->cc.cycle_tm2 = nd_max(ep->rtt, 1000);
		if (ep->cc.last_loss_tm + ep->cc.cycle_tm2 < curr) {
			double factor = 1.02;
			double old_inter_usec = ep->cc.inter_usec;
			...
			ep->cc.cwnd2 += (curr - ep->cc.last_cycle_wnd_tm) / old_inter_usec;
			ep->cc.last_cycle_wnd_tm = curr;

			ep->cc.cwnd = ep->cc.cwnd2 + (ep->cc.cycle_tm + ep->rtt - (curr - ep->cc.begin_tm))/ep->cc.inter_usec;
			//////////////////////////////////////////////////////////////////////////
			//ep->cc.cwnd = ep->cc.cwnd * 0.9;
		}
		return;
	}
	if (ep->cc.last_loss_tm + ep->cc.cycle_tm/4 < curr) {
		double factor = 1.008;
		double old_inter_usec = ep->cc.inter_usec;
		if (ep->cc.inter_usec > 1000) {
			factor = 1.02;
...
nd_rudp_dump_trace_info(nd_rudp_endpoint_t *ep)
{
	...
	nd_rudp_log(ND_RUDP_LOG_LV_DUMP, 
		"dump trace: "
		"rtt=%u, cwnd=%.2f, cc.left_wnd=%d, inter_usec=%.2f, cc.rate=%u, cc.bw=%u, cc.last_loss_rate=%.2f, "
		"s_next_seq=%u, s_next_ack_seq=%u, s_max_seq=%u, "
		"r_next_seq=%u, r_last_seq=%u, r_last_max_seq=%u, r_next_staged_ack_seq=%u, "
		"s_que.size=%u, r_que.size=%u, s_buf_lst_size=%u, r_buf_lst_size=%d, s_que.free_size=%u\r\n",
		ep->rtt, ep->cc.cwnd, ep->cc.left_wnd, ep->cc.inter_usec, ep->cc.rate, ep->cc.bw, ep->cc.last_loss_rate,
		ep->s_next_seq, ep->s_next_ack_seq, ep->s_max_seq,
		ep->r_next_seq, ep->r_last_seq, ep->r_last_max_seq, ep->r_next_staged_ack_seq,
		ep->s_que.size, ep->r_que.size, ep->s_buf_lst_size, ep->r_buf_lst_size, ep->s_que.free_size);
}
