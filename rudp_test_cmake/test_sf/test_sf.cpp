...
#include "../rudp/nd_core/nd_time.h"
...
{
	enum {MAX_BUF_SIZE = 32 * 1024, HDR_LEN = 2 + 2};
public:
	snd_file()
	{	
...
public:
	...
	bool folder_send(uint64_t &send_total_size, unsigned long &tm)
	{
		...
		*((uint16_t*)(buf + HDR_LEN + 8)) = htons(short_fn.size());
		...
		if (ndrudp_send(this->s_, buf, len, -1, ND_RUDP_S_FLAG_SND_N) != len)   
		{
			printf("send error\r\n");
			return false;
		}
				...
				*((uint16_t*)(buf + HDR_LEN + 8)) = htons(short_fn.size());
				...
				while (1)
				{
					...
					*((uint16_t*)buf) = htons(rd_len);
					...
					int snd_len = ndrudp_send(this->s_, buf, rd_len + HDR_LEN, -1, ND_RUDP_S_FLAG_SND_N);
	...
	bool __svc()
	{
		...
		if (HDR_LEN != ndrudp_recv(this->s_, buf, HDR_LEN, -1, ND_RUDP_R_FLAG_RCV_N) || buf[2] != 3) {
